...
from pointcloud_dataset import read_las, PointCloudsInFiles
...
def net_train(model, train_loader, device, optimizer, train_weights):
        model.train()
        losses = []
        for i, data in enumerate(tqdm(train_loader, desc='Training in progress')):
            ...
            loss = F.nll_loss(out, data.y.long(), weight=train_weights)
...
def main():
    ...
    # coords, attrs = read_las("lsc_33412_5658_2_sn.las", get_attributes=True)
    coords, attrs = read_las("pc2011_10245101_sub.las", get_attributes=True)

    # for a in attrs:
    #     print(f"{a}: {attrs[str(a)]}")
    ...
    train_weights = torch.Tensor(class_weights).to(device)
    # print(f"Class weights: {train_weights}")

    train_dataset = PointCloudsInFiles("/workspace/train/", glob='*', column_name='classification', max_points=20)
    train_loader = DataLoader(train_dataset, batch_size=16, shuffle=False, num_workers=5)
    
    meta = OrderedDict()
    last_epoch = -1
    print("Starting training from scratch.")

    model = PN2_Classification(num_features=0, num_target_classes=len(unique_classes)).to(device)
    ...
    for epoch in range(last_epoch+1, 1001):
        # meanloss = net_train()
        meanloss = net_train(model, train_loader, device, optimizer, train_weights)
        print(meanloss)
        torch.save(model.state_dict(), str("best.model").format(epoch=epoch))
